...
public class TourRepository : ITourRepository
{
    ...
    /// <summary>Initializes a new instance of the <see cref="TourRepository"/> class.</summary>
    public TourRepository(TourBookingDbContext context, ILogger<TourRepository> logger)
    {
        _context = context;
        _logger = logger;
    }
    ...
    public async Task<IEnumerable<Tour>> SearchAsync(string searchTerm, CancellationToken cancellationToken = default)
    {
        try
        {
            return await _context.Tours
                .AsNoTracking()
                .Where(t => t.TourName.Contains(searchTerm) || t.Place.Contains(searchTerm))
                .ToListAsync(cancellationToken);
        }
        catch (Exception ex)
        {
